...
- **The fill is the state signal, solid.** `.flat-chip-active` paints `--accent-text` at full alpha: a near-black pill on the light ground, near-white on the dark one, since that token inverts per theme. Active vs inactive measures 14.24:1 in light and 16.03:1 in dark, and the pill clears its page at 15.83:1 / 17.69:1.
- **The label must invert with it** — `text-surface` (19.29:1 light, 15.60:1 dark). `text-accent` on a filled pill is the neutral on the neutral and is invisible; `text-ink` fails the same way on the light side.
- This does not compete with the primary button: the pill is neutral, and `bg-accent-fill` green is the only green fill in the app.
