...
- **The accent is the brand's own darkest step, not a second hue.** `--accent` is `oxblood-1000` (`20 1 1`, near-black) in light and inverts to `oxblood-50` (`253 236 236`, near-white) in dark — a near-black fill needs a ground to sit on, and the dark page is already near-black. So unlike `--danger`, the accent **flips between themes**: never assume `bg-accent-fill` is light-on-dark or dark-on-light, pair it with `text-accent-on` and let the tokens sort it out.
- **Fill** (buttons, badges, progress bars, active-pill highlights): `bg-accent-fill`, `border-accent-fill`, `ring-accent-fill` (and `-fill` for danger — danger stays the same red in both themes). Text on an accent fill: `text-accent-on`.
- **Bare text/icon** (needs to read directly against a background, not sit on a solid fill): `text-accent` / `text-danger` — these are theme-adaptive. `--accent` and `--accent-text` now point at the same ramp step (a neutral this far up the ramp doesn't need the darkening the green did), but keep using the right one — the split is what let the accent change hue without auditing every call site.
- **Accent no longer marks a highlight on its own.** In light mode `text-accent` (`20 1 1`) lands within a point of `text-ink` (`26 14 15`); in dark it's a hair off white. Weight, fill and border carry emphasis now — reaching for `text-accent` to make something stand out is a no-op.
...
- Platform manifests can't read CSS variables, so `lib/brand.ts` holds those literals in one place. Keep it in sync with `--background`; its `accent` carries the **dark-mode** value, since everything reading it is the dark shell.
