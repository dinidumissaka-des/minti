...
hooks/
  ...
  useRates.ts       Rates for one display currency; reports failure rather than throwing
...
-- income_entries carries `currency` too. user_settings carries `base_currency`:
-- what an amount with no currency of its own is counted in.
```
RLS enabled on both tables. `billing_day` exists in DB but is hidden from UI (hardcoded to 1).

## Key patterns
- **An amount is a number *and* a currency**: every entry stores what it was entered in (`currency`, null meaning the account's `base_currency`), and `lib/money.ts` converts a fetched page of rows **once, at the boundary** — `page.tsx` for the month's expenses and bills, `AnalyticsView` for the previous month, `IncomeSection` for its entries. Everything downstream therefore sums a single currency and needs no conversion of its own. A converted row keeps what was entered in `original`, which rows render beneath the converted figure and edit forms load into the amount field — **editing must never write a converted figure back**. Saved settings (`budget`, `monthly_income`) are held in the base currency and converted for display, so their editors are labelled with `money.base`, not the display currency. When a rate is missing, `convert` returns the amount unchanged and `hasUnconverted` says so, which the page surfaces as a notice — never a silently wrong total.
